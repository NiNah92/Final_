<template>
 ...
 <main-header navsel="back"></main-header></div>
  ...
  <button class="btn btn-success btn-sm" v-on:click="navigateTo('/blog/create')">Create product</button> 
  <strong> จํานวน product: </strong> {{results.length}}
  </div>
  <ul class="categories">
 ...
 href="#">Clear</a></li>
  ...
  <p>
  <button class="btn btn-sm btn-info" v-on:click="navigateTo('/blog/'+ blog.id)">View details</button>
  <button class="btn btn-sm btn-warning" v-on:click="navigateTo('/blog/edit/'+ blog.id)">Edit product</button>
  ...
  <div v-if="blogs.length === 0 && loading === false" class="empty-blog"> *** ไม่มีสินค้า*** </div>
